...
climdex.pcic.test.select.blocks <- function() {
  ...
  dat1 <-
    c(rep(TRUE, 4), FALSE, rep(TRUE, 3), rep(FALSE, 2), rep(TRUE, 5))
  result1 <-
    c(rep(TRUE, 4), rep(FALSE, 4), rep(FALSE, 2), rep(TRUE, 5))
  dat2 <-
    c(FALSE,
      rep(TRUE, 4),
      FALSE,
...
climdex.pcic.test.total.precip.op.threshold <- function() {
  ...
  checkEquals(as.numeric(
    total.precip.op.threshold(prec.dat, phony.date.factor, 1, ">=")
  ), c(10.5, 11))
}
...
climdex.pcic.test.spell.length.max <- function() {
  ...
  cwd <-
    as.numeric(spell.length.max(prec.dat, phony.date.factor, 1, ">=", TRUE))
  
  ## Without spells spanning years...
  altcwd <-
    as.numeric(spell.length.max(prec.dat, phony.date.factor, 1, ">=", FALSE))
  
  checkEquals(cwd.result, cwd)
  checkEquals(altcwd.result, altcwd)
}

climdex.pcic.test.simple.precipitation.intensity.index <-
  function() {
    prec.dat <- c(0.1, 3.0, 4.3, 0.9, 1.3, 6.0, 0, 0, 4.0, 1)
    phony.date.factor <- factor(rep(1:2, each = 5))
    sdii <-
      as.numeric(simple.precipitation.intensity.index(prec.dat, phony.date.factor))
...
climdex.pcic.test.threshold.exceedance.duration.index <-
  function() {
    prec.dat <- c(0.1, 3.0, 4.3, 1.9, 1.3, 6.0, 0, 0, 4.0, 1)
    phony.date.factor <- factor(rep(1:2, each = 5))
    ...
    ## With spells spanning years...
    alttedi <-
      threshold.exceedance.duration.index(prec.dat,
                                          phony.date.factor,
                                          phony.jdays,
                                          thresholds,
                                          op,
...
climdex.pcic.test.percent.days.op.threshold <- function() {
  is.x86 <-
    !(climdex.quantile(c(0, 1, 2), 0.3) == quantile(c(0, 1, 2), 0.3, type =
                                                      8))
  
  ## Parse the dates into PCICt.
  tmax.dates <-
    as.PCICt(do.call(paste, ec.1018935.tmax[, c("year", "jday")]),
             format = "%Y %j",
             cal = "gregorian")
  tmin.dates <-
    as.PCICt(do.call(paste, ec.1018935.tmin[, c("year", "jday")]),
             format = "%Y %j",
             cal = "gregorian")
  prec.dates <-
    as.PCICt(do.call(paste, ec.1018935.prec[, c("year", "jday")]),
             format = "%Y %j",
             cal = "gregorian")
  
  ## Load the data in.
  ci <- climdexInput.raw(
    tmax = ec.1018935.tmax$MAX_TEMP,
  ...
  tx90p <- percent.days.op.threshold(
    ci@data$tmax,
    ci@dates,
    ci@jdays,
...
climdex.pcic.test.growing.season.length <- function() {
  ...
  test.dates <-
    seq(as.PCICt("1961-01-01", cal = ls),
        as.PCICt("1961-12-31", cal = ls),
        by = "days")
  ...
  res <-
    sapply(num.days.set, function(num.days) {
      sapply(td, function(dat) {
        sapply(modes, function(mode) {
          growing.season.length(dat,
                                test.dates.factor,
                                test.dates,
                                TRUE,
                                num.days,
...
climdex.pcic.test.bootstrap.quantiles <- function() {
  ...
  get.quantile.for.day <-
    function(ci,
      ...
      dat <-
        ci@data[[var]][climdex.pcic:::get.bootstrap.set(ci@dates, ci@base.range, win.size)]
      ...
      num.yrs <- dim(dat)["yr"]
      yr.idx <-
        if (missing(rm.yr.idx) ||
            missing(dup.yr.idx))
          1:num.yrs
      else
        c((1:num.yrs)[-rm.yr.idx], dup.yr.idx)
      return(quantile(
        dat[day.idx, yr.idx],
        probs = q,
        type = 8,
  ...
  check.inbase.quantiles.for.day <- function(ci, pctile, var, day) {
    climdex.pcic.data <-
      ci@quantiles[[var]]$inbase[[paste("q", pctile, sep = "")]][day, , ]
    nyr <- dim(climdex.pcic.data)[1]
    comparison.data <-
      t(sapply(1:nyr, function(x) {
        sapply((1:nyr)[-x], function(y) {
          get.quantile.for.day(ci, 5, pctile / 100, var, day, x, y)
        })
      }))
  ...
  check.outbase.quantiles <- function(ci, pctile, var) {
    climdex.pcic.data <-
      ci@quantiles[[var]]$outbase[[paste("q", pctile, sep = "")]]
    comparison.data <-
      sapply(1:length(climdex.pcic.data), function(x) {
        get.quantile.for.day(ci, 5, pctile / 100, var, x)
      })
    dimnames(comparison.data) <- NULL
    names(comparison.data) <- NULL
    checkEquals(climdex.pcic.data, comparison.data)
